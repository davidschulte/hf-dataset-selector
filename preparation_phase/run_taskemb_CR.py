...
from transformers_taskemb import (WEIGHTS_NAME,
                          BertConfig_TaskEmbeddings as BertConfig,
                          BertForSequenceClassification_TaskEmbeddings as BertForSequenceClassification,
                          BertTokenizer)

from transformers_taskemb import AdamW, get_linear_schedule_with_warmup

from transformers_taskemb import glue_compute_metrics as compute_metrics
from transformers_taskemb import glue_output_modes as output_modes
from transformers_taskemb import glue_processors as processors
from transformers_taskemb import glue_convert_examples_to_features as convert_examples_to_features
...
def compute_taskemb(args, train_dataset, model, disable_tqdm=True):
    """ Feed task data through the model """
    # tb_writer = SummaryWriter()

    train_sampler = RandomSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size,
                                  collate_fn=(lambda x: train_dataset.collate_fn(x, return_token_type_ids=True)))
    ...
    model.zero_grad()
    train_iterator = trange(int(args.num_epochs), desc="Epoch", disable=disable_tqdm)
...
def get_args():
    ...
    parser.add_argument("--gpu", default='1', type=str, required=False,
                        help="GPU device to be used. Example: 2.")
    parser.add_argument("--scenario", default='sources', type=str, required=False,
                        help="Source or Target embeddings")
    parser.add_argument("--dataset_start_idx", default=None, type=int, required=False,
                        help="Where in the dataset list to start")
    parser.add_argument("--dataset_end_idx", default=None, type=int, required=False,
                        help="Where in the dataset list to end")
